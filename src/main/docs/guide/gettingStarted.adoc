...
[NOTE]
====
For Kotlin the `openapi` dependency should be in the `kapt` or `ksp` scope, for Groovy in the `compileOnly` scope and for Python in the regular dependencies (the Python compiler runs the annotation processors found on the compile classpath).
====
