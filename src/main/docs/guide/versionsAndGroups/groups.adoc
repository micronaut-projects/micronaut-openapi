  ...
  openapi:
    groups:
      v1:
        ...
        # A flag indicating that the Swagger UI / Scalar should select this group in the selector by default.
        primary: true
        # The name of the final OpenAPI spec file. If it is not set, then by default the name will be generated according
...
After processing the annotation processor, you will get 3 different OpenAPI files.

== Integration with UI

In RapiDoc, ReDoc and OpenAPI Explorer do not support multiple files at the same time,
so for normal API display, you will have to complete the template yourself.

*Swagger UI* and *Scalar* support multiple files out of the box. The example above would look something like this:
